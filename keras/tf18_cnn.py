...
w1 = tf.get_variable('w1', shape=[3,3,1,32])  # 위와 같은 것이지만 이것이 더 많이 쓰인다.
                        # kenel_size,kenel_size,input(*),output
L1 = tf.nn.conv2d(x_img,w1,strides=[1,1,1,1], padding='SAME')
L1 = tf.nn.selu(L1)
L1 = tf.nn.max_pool(L1, ksize=[1,2,2,1], strides=[1,2,2,1],padding='SAME')

w2 = tf.get_variable('w1', shape=[3,3,32,64])  # 위와 같은 것이지만 이것이 더 많이 쓰인다.
                        # kenel_size,kenel_size,input(*),output
...
w5 = tf.get_variable('w5', shape=[7*7*64, 10], initializer=tf.contrib.layers.xavier_initializer())
b5 = tf.Variable(tf.random_normal([10]))
hypothesis = tf.nn.softmax(tf.matmul(L2_flat, w5) + b5)
