...
R2는  0.9999999999153343 튜닝 후 바꿔야함
...
x1_train, x1_test, y1_train, y1_test = train_test_split(
    x1, y1, random_state=66, test_size=0.23 , shuffle=False
)

from sklearn.model_selection import train_test_split
x2_train, x2_test, y2_train, y2_test = train_test_split(
    x2, y2, random_state=66, test_size=0.23 , shuffle=False
)
...
dense1_1 = Dense(5, activation='relu', name='aaa')(input1) # input 레이어를 알려줘야 함/ 노드를 5개로 가지고 input1을 인풋레이어로 받는 댄스층
dense1_2 = Dense(8, activation='relu', name='bbb')(dense1_1) 
dense1_3 = Dense(32, activation='relu', name='ccc')(dense1_2)
dense1_4 = Dense(8, activation='relu', name='ddd')(dense1_3)
#output1 = Dense(3)(dense4) 아웃풋 밑에서 묶어 줌

# 함수형 모델 2
input2 = Input(shape=(3, ), name='input2')
dense2_1 = Dense(5, activation='relu', name='aaa1')(input2) # input 레이어를 알려줘야 함/ 노드를 5개로 가지고 input1을 인풋레이어로 받는 댄스층
dense2_2 = Dense(8, activation='relu', name='bbb1')(dense2_1)
dense2_3 = Dense(32, activation='relu', name='ccc1')(dense2_2)
dense2_4 = Dense(8, activation='relu', name='ddd1')(dense2_3)
#output2 = Dense(3)(dense2_4)

# 엮어주쟈~~~ ***************************************************
from keras.layers.merge import concatenate
merge1 = concatenate([dense1_4,dense2_4], name='111') #output 자체를 묶어준다
middle1 = Dense(32, name='222')(merge1)
middle2 = Dense(32, name='333')(middle1)
middle3 = Dense(32, name='444')(middle2)
################ output 모델 구성 #################

# 함수형 모델 1
output1 = Dense(16, name='aaaa')(middle3)
output1_2 = Dense(8, name='bbbb')(output1)
output1_2 = Dense(3, name='cccc')(output1_2)

# 함수형 모델 2
output2 = Dense(16, name='aaaa1')(middle3)
output2_2 = Dense(8, name='bbbb1')(output2)
output2_3 = Dense(3, name='cccc1')(output2_2)

model = Model(inputs=[input1, input2],
                outputs=[output1_2,output2_3])
...
model.fit([x1_train,x2_train],[y1_train,y2_train], epochs=500, batch_size=5,
         validation_split = 0.25, verbose=1) # verbose 학습중의 정보를 보여주는 버전 고르기 0(아예안나옴),1(다나옴),2(화살표는 안나옴),3(epo만 나옴) 딜레이 시간 아낄려고 -> 1/2 쓰면 될듯
        

#4. 평가
evaluate = model.evaluate([x1_test,x2_test],[y1_test,y2_test], batch_size=1)

a = model.evaluate([x1_test,x2_test],[y1_test,y2_test], batch_size=5) # 통상적으로 디폴트값 쓰긴하는데, fit이랑 맞춰줘야 하지 않겠나 싶어서 선생님은 fit값이랑 맞춰준다.
print('=============================================================')
print('a 는 ',a)
...
MSE2 = mean_squared_error(y1_test, y_predict[1])
MSE = (MSE1+MSE2)/2
