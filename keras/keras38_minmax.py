...
scaler.fit(x) # MinMaxScaler 전처리를 실행하다 
x = scaler.transform(x) # 실행한 값을 변환해라
x_predict = scaler.transform(x_predict) # 실행한 값을 변환해라
# 이 전처리 할때 input np.array가 2차원을 원하네
print(x)
print(x_predict)

# scaler.fit(y) # MinMaxScaler 전처리를 실행하다 
