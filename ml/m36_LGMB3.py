...
# model = LGBMClassifier(objective='multi:softmax', estimators=300, learning_rate=0.01, n_jobs=-1) # 나무의 개수는 결국 epo다
# model.fit(x_train, y_train, verbose=True, eval_metric=['merror'], eval_set=[(x_train,y_train),(x_test,y_test)]
#         , early_stopping_rounds=20) 
model = LGBMClassifier(objective= 'multiclass', estimators=300, learning_rate=0.01, n_jobs=-1) # 나무의 개수는 결국 epo다
model.fit(x_train, y_train, verbose=True,eval_metric=["multi_logloss","multi_error"], eval_set=[(x_train,y_train),(x_test,y_test)]
        , early_stopping_rounds=20) 
...
modelpath = './model/xgb_save/auc' + str(score*100.0) + '.dat'
